...
export const beat = (phase: string, cycle: number, mined: number): void => {
  const time = now();
  ...
  lastBeat = time;
  log(
    `mining: still here - ${phase}, cycle ${cycle}, at ${player.x},${player.y}, ` +
      `${player.weight}/${player.weightMax}, ${mined} swings`,
  );
};
