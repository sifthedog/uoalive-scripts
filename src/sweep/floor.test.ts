...
import { AMMO_GRAPHICS, BLOCKED_DELAY, COIN_GRAPHICS, GRAB_RANGE } from './config.js';
...
describe('onFloor', () => {
  ...
  it('collects gold as readily as ammunition', () => {
    world.client.findAllItemsOfType = answering({
      [GOLD]: [item({ serial: 3, graphic: GOLD, container: 0, amount: 312 })],
    });

    expect(onFloor().map((found) => found.serial)).toEqual([3]);
  });

  it('leaves the gold already in your pack alone', () => {
    world.client.findAllItemsOfType = answering({
      [GOLD]: [item({ serial: 3, graphic: GOLD, container: 0x40000000 })],
    });
...
describe('describeFloor', () => {
  ...
  // A shard whose coin art is not 0x0eed is otherwise an afternoon of gold left on the floor
  it('counts the coin graphic in the census too', () => {
    expect(describeFloor()).toContain(hex(GOLD));
  });
});
