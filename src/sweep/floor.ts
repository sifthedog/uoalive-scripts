...
import { BLOCKED_DELAY, SWEEP_GRAPHICS } from './config.js';
...
export const inReach = (items: Item[], range: number): Item[] =>
  items.filter((item) => distanceTo(item) <= range && !isBlocked(item.serial));
...
// A stack the client puts in reach and the server will not move. Timed rather than permanent: what
// refused it is usually another player standing on it.
export const setAside = (items: Item[]): void => {
...
// Stacks are picked up and decay, so the map would grow for as long as the client runs
