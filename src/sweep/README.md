# sweep — picking ammunition and gold up off the floor

Builds one script:

| Script | What it does |
| --- | --- |
| `dist/sweep.js` | Watches the ground and moves every arrow, crossbow bolt and gold pile within reach into your backpack |

## Why it exists

Shooting a butte scatters arrows across the ground, and what you kill between shots drops gold on
the same floor. The client gives you no way to sweep either up — every stack is a drag into the
pack. This is that sweep, on a loop, so it can be left running while you shoot.
...
3. **Scan the ground** for every graphic in `SWEEP_GRAPHICS` and keep what is within `GRAB_RANGE`.
...
Coins are tallied apart from everything else — `took 12 arrows and 312 gold in 3 stacks` — because
the two amounts summed into one number mean nothing.

### Before you paste it

- **Nothing walks.** Arrows that flew past the target stay where they landed — step over to them and
  the next cycle takes them.
- **It takes any gold on the floor within reach**, including a pile another player has just dropped.
  There is no ownership check anywhere in this script.
- **Gold is heavy.** Arrows made `pack is full` the likely ending; gold makes it overweight instead.
  `WEIGHT_BUFFER` is the one default worth raising for a long run.
- **Gold stacks**, so a pile joining the one already in your pack costs no top-level slot.
  `PACK_LIMIT` does not bind on gold the way it binds on arrows.
- **Arrows and gold inside a corpse are not taken.** They have a container, so they are not on the
  floor. [`carve`](../carving/README.md) is the script that opens those.
- Anything already in your pack or in a bag is left alone, for the same reason.
...
Then paste `dist/sweep.js` into the client's script editor. The first line is a census — how many of
...
| `COIN_GRAPHICS` | `0x0eed` | Gold. Delete the line to stop taking it; anything listed here is counted as coins rather than as items |
...
**`0x0f3f x0, 0x1bfb x0, 0xeed x0 in the world`** with arrows or gold plainly on the floor — the
graphics are wrong for this shard. Hover one and read its art, then correct the list it belongs to.
...
**`overweight`** — usually the gold. Raise `WEIGHT_BUFFER` or bank what you have.

**It sits on `still here - watching` while stacks are on the floor** — they are further than
...
- **`client.findAllItemsOfType(graphic, undefined, 'world')` is the whole of the ground scan.** There
  ...
  why `SWEEP_GRAPHICS` is a list rather than a filter.
- **A ground item's `container` is 0 *or* the world serial `0xffffffff`**, depending on the client,
  ...
  both count. That field is the whole ground filter, and it is what keeps the arrows and the gold
  already in your own pack out of a sweep.
...
- **Every graphic.** `0x0f3f`, `0x1bfb` and `0x0eed` are the stock arrow, crossbow bolt and gold
  coin arts. The startup census is there to catch a shard where they are not.
- **That gold reaches the floor at all.** Some shards send it straight to the bank, in which case the
  sweep simply never sees a pile.
