...
class Pace(object):
    """The shard's own skill timer, learned rather than configured."""
...
class Heartbeat(object):
    ...
    def beat(self, phase, cycle, tally):
        ...
        self._last = moment
        log(
            "still here - %s, cycle %d, at %d,%d, %d/%d, %d attempts"
            % (
                phase,
                cycle,
                API.Player.X,
...
stall = StallWatch("cycles without an attempt", STALL_WARN, STALL_STOP, heartbeat)
...
def wait_out_save():
    ...
    # Read before the clear: a save can start and finish inside one attempt, and clearing first
    # threw the completion away and then stood still for the whole of SAVE_WAIT
...
def stop_reason():
    ...
    # HitsMax and FollowersMax read 0 before the client has been told, the way maxMana does
    ceiling = API.Player.HitsMax

    if ceiling > 0 and API.Player.Hits < ceiling * HEALTH_FLOOR:
        return "hurt (%d/%d)" % (API.Player.Hits, ceiling)

    slots = API.Player.FollowersMax

    # Keeping the tames fills the slots, and a shard with no room left refuses every attempt
    # without saying why
    if slots > 0 and API.Player.Followers >= slots:
...
def skill_value():
    ...
    # Value is a float percentage (74.6), and reads 0.0 until the skill list arrives - which is a
    # real value too, so it is reported as unknown rather than coerced
...
def leave_out(serial):
    skipped.add(serial)


def mine(name):
    return PET_NAME != "" and name.lower() == PET_NAME.lower()
...
def next_quarry(graphic):
    if HUNT_RADIUS <= 0:
        return None

    candidates = [
        mobile
        for mobile in API.GetAllMobiles(graphic=graphic, distance=HUNT_RADIUS)
        if not mobile.IsDestroyed
        and mobile.Serial not in skipped
...
def chase(serial):
    ...
    if API.PathfindEntity(serial, TAME_RANGE, True, CHASE_TIMEOUT, True):
        return "closed"
...
# Called between the slices of a taming wait, so the animal is followed while the attempt resolves
def keep_up(serial):
    found = find_mobile(serial)

    if found is not None and found.Distance > TAME_RANGE and not API.Pathfinding():
        API.PathfindEntity(serial, TAME_RANGE, False, CHASE_TIMEOUT, True)
...
def wait_out(buckets, budget, between=None):
    waited = 0.0
    ...
    while waited < budget:
        ...
        API.Pause(TAME_WAIT_SLICE)
        waited += TAME_WAIT_SLICE

    return None


def settle(serial, was_pet, between):
    first = wait_out(OUTCOME_TEXT, TAME_START_TIMEOUT, between)

    if first is not None and first != "starting":
        return first

    resolved = wait_out(RESOLUTION_TEXT, TAME_RESOLVE_TIMEOUT, between)
...
def tame_once(serial, between):
    ...
    try:
        return settle(serial, was_pet, between)
    finally:
...
def rename_pet(serial, name):
    ...
    for _ in range(RENAME_ATTEMPTS):
        ...
        if settled(RENAME_TIMEOUT, RENAME_POLL, answers_to):
            return "renamed"
...
def context_menu(serial, texts):
    for text in texts:
        if API.ContextMenu(serial, text, CONTEXT_TIMEOUT):
            return True
...
confirm_button = None
said_no_gump = False
said_no_button = False


# The confirm is the gump id *changing*. WaitForGump with no id resolves to whatever LastGumpID
# already is, so it answers a stale gump when one is open and times out when none is.
def await_confirm(before):
    waited = 0.0

    while waited < RELEASE_CONFIRM_TIMEOUT:
        found = API.HasGump()

        if found and found != before:
            return found

        API.Pause(RELEASE_CONFIRM_POLL)
        waited += RELEASE_CONFIRM_POLL

    return 0


def answer_confirm(before, button):
    global said_no_gump, said_no_button

    gump = await_confirm(before)

    if not gump:
        if not said_no_gump:
            said_no_gump = True
            log("found no gump to confirm the release with")

        return

    if not said_no_button and not any(
        API.GumpContains(text, gump) for text in RELEASE_CONFIRM_TEXT
    ):
        said_no_button = True
        log("the release gump says none of RELEASE_CONFIRM_TEXT - answering it anyway")
...
def release_pet(serial):
    ...
    candidates = list(RELEASE_CONFIRM_BUTTONS) if confirm_button is None else [confirm_button]
    pressed = False
    missing = 0

    for button in candidates * RELEASE_ATTEMPTS:
        before = API.HasGump()

        if not context_menu(serial, RELEASE_MENU_TEXT):
            # Better proof than the flag, and free: the entry is on the menu only while it is your
            # pet, so one that has gone since a press means the press worked
            if pressed:
                return "released"
            ...
            if missing >= RELEASE_ATTEMPTS:
                return "noEntry"

            API.Pause(MENU_RETRY_DELAY)
            continue

        pressed = True
        answer_confirm(before, button)

        if settled(RELEASE_TIMEOUT, RELEASE_POLL, lambda: not is_pet(serial)):
            if confirm_button is None:
                confirm_button = button
                log("the release gump answers to button %d" % button)
...
def command_kill(serial, name):
    if API.HasTarget():
        API.CancelTarget()

    if not context_menu(serial, KILL_MENU_TEXT):
        return "noEntry"

    if not API.WaitForTarget("any", KILL_CURSOR_TIMEOUT):
        return "noCursor"

    log("told '%s' to kill - pick its target" % name)

    picked = settled(KILL_PICK_TIMEOUT, KILL_PICK_POLL, lambda: not API.HasTarget())
...
# Renamed before anything else is done with it: once it is not your pet the Rename packet is
# refused
def after_tame(serial, name):
    ...
    if PET_NAME:
        if rename_pet(serial, PET_NAME) == "renamed":
            called = PET_NAME
    ...
    if AFTER_TAME == "kill":
        ordered = command_kill(serial, called)
...
while stop is None:
    ...
    sighted = None if hunting is None else next_quarry(hunting)
    ...
    if quarry is None:
        ...
        quarry = {"serial": picked, "name": named(animal), "graphic": animal.Graphic}
    ...
    for cycle in range(MAX_CYCLES):
        ...
        outcome = tame_once(quarry["serial"], lambda: keep_up(quarry["serial"]))
    ...
    leave_out(quarry["serial"])

reason = stop or "the session ended"

log(
    "%d tamed over %d attempts, %d failed, %s %s -> %s"
    % (tamed, attempts, failures, SKILL_NAME, reading(start), reading(skill_value()))
)
