# Built from src/inscription/index.py by build.py - do not edit.
...
# src/inscription/index.py
log = make_log("inscription")
heartbeat = Heartbeat(HEARTBEAT_EVERY, log, "made", position_and_mana)
...
# Only the kinds the band spends: pulling garlic for a lightning scroll is weight and nothing else
def targets_for(item):
    return dict((kind, BATCH_SIZE) for kind in NEEDS[item])
...
# The band's scroll only: a mage may take a lightning scroll and refuse the reveal left from before
...
tools = CraftTool("scribe's pen", TOOL_GRAPHICS, TOOL_NAME_WORDS, log)
stock = StockBook({
...
# Before the cursors: a capped character has nothing to pick containers for
...
if start is None:
    ...
    log("%s is at %.1f and the table starts at %.1f - buy it up from an NPC mage first"
...
if tools.serial() is None:
    log("no scribe's pen in the pack")
    API.Stop()
...
# A run that starts on what it is already carrying needed no cursor at all
if len(sources.picked()) == 0 and crafts_left(first) == 0:
    log("nothing picked, and the pack is short of %s for one %s"
        % (shortfall_report(short_for(first), KIND_ORDER), first))
    API.Stop()
...
if output == "sell":
    log("selling every %d to a %s" % (SELL_AT, VENDOR_NOUN))
elif output != "unload":
    output = "keep"
    log("keeping the scrolls - the run ends once the pack holds %d" % MAX_HELD)
...
try:
    while stop is None and cycle < MAX_CYCLES:
        ...
        if API.Player.Mana < MANA[product]:
            if not regain_mana(MANA[product]):
                dry += 1
                log("mana did not come back (%d/%d)" % (dry, MAX_DRY))

                if dry >= MAX_DRY:
                    stop = "mana is not coming back - %d/%d, and one %s takes %d" % (
        ...
        if outcome in ("made", "failed"):
            ...
            # The pool was gated on MANA before the craft, so this row's figure understates the cost
            log("refused for mana at %d - raise MANA_BY_CIRCLE for %s" % (API.Player.Mana, product))
            regain_mana(MANA[product])
            stall.progressed()
            ...
            if pulled > 0:
                ...
                # Everything the recipe takes is in the pack and refused: the row is another spell
                no_material += 1
            ...
            crafter.forget_last()
            stall.progressed()
            log("the pen wore out, looking for another")
            ...
            if no_tool >= MAX_NO_TOOL:
                stop = ("no scribe's pen left" if outcome == "noTool"
                        else "the craft menu will not open")
                break

            log("%s (%d/%d), trying again"
                % ("no scribe's pen in the pack" if outcome == "noTool"
                   else "the pen opened no craft menu", no_tool, MAX_NO_TOOL))
        ...
        if tally >= reported + LOG_EVERY:
            reported = tally
            log(
                "%d made, %d failed, %d throttled, %s at %s, %s left, %d/%d mana"
