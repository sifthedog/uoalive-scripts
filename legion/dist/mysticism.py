# Built from src/mysticism/index.py by build.py - do not edit.
...
# target_kind is the cursor the shard raises - the area rows are harmful, and a pre-target set to
# the wrong kind does not fire at all. cast_timeout and cast_delay are per row because the spells
# are seconds apart, and each timeout is the measured cast with a margin.
STAGES = [
    # Cheapest in the book, and the only band with no cursor to answer: the mana goes and the skill
    # is rolled as the incantation ends, so the cursor it raises can be left for the next cycle to
    # cancel. Below about 30 the sensible thing is to buy the skill from an NPC trainer.
    {
        "up_to": 40.0,
        "spell": "Nether Bolt",
        "mana": 4,
    ...
    # An area spell centred on the caster, so this band belongs somewhere empty
    {
        "up_to": 95.0,
        "spell": "Hail Storm",
        "mana": 40,
...
# The floor of the first band. Under it the book refuses everything, so the run says so rather than
# grinding out refusals.
FIRST_BAND = 40.0
...
# Gating on the buff would cap the run at one cast per Stone Form
...
# The two area bands are harmful spells centred on the caster, and nothing here heals - so a health
# floor is a stop rather than a pause
HURT_FLOOR = 0.5
...
OUTCOME_TEXT = [
    ...
    ("disabled", ["You are no longer in stone form", "You are no longer", "You have dispelled"]),
    # Stone Form shuts the rest of the book on some shards, which strands the bands above it
